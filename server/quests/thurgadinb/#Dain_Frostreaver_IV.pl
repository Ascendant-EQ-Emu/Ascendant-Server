...
sub EVENT_KILLED_MERIT {
	...
	quest::set_data("velious_dain_" . $account_id, $char_name);

	my $first_key = "first_kill_dain";
	unless (quest::get_data($first_key)) {
		quest::set_data($first_key, $char_name . "|" . $uguild);
		quest::we(15, "SERVER FIRST! " . $char_name . " <" . $uguild . "> and their group have slain Dain Frostreaver IV for the first time on this server!");
	}
}
