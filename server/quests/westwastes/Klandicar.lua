function event_killed_merit(e)
	...
	eq.set_data("velious_klandicar_" .. account_id, char_name);

	local first_key = "first_kill_klandicar";
	if (eq.get_data(first_key) == "") then
		eq.set_data(first_key, char_name);
		eq.world_emote(15, "SERVER FIRST! " .. char_name .. " and their group have slain Klandicar for the first time on this server!");
	end
end
