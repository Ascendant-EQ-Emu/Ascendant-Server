...
function event_killed_merit(e)
	...
	eq.set_data("velious_kingtormax_" .. account_id, char_name);

	local first_key = "first_kill_kingtormax";
	if (eq.get_data(first_key) == "") then
		eq.set_data(first_key, char_name);
		eq.world_emote(15, "SERVER FIRST! " .. char_name .. " and their group have slain King Tormax for the first time on this server!");
	end
end
