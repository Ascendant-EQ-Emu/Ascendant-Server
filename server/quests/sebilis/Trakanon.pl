sub EVENT_KILLED_MERIT {
	...
	quest::set_data("kunark_trakanon_" . $account_id, $char_name);
	
	my $first_key = "first_kill_trakanon";
	unless (quest::get_data($first_key)) {
		quest::set_data($first_key, $char_name . "|" . $uguild);
		quest::we(15, "SERVER FIRST! " . $char_name . " <" . $uguild . "> and their group have slain Trakanon for the first time on this server!");
	}
}