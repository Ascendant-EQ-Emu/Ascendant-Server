...
RULE_BOOL (World, IPLimitDisconnectSameAccount, true, "When IP limit is exceeded, first evict existing sessions from the same account before applying broader IP limit rules. This allows players to quickly reconnect after a crash without affecting other accounts on the same IP.")
...
RULE_BOOL(Combat, RiposteAASkillStacking, true, "When true, multiple riposte-with-skill AAs (Return Kick, Furious Refrain, Knave's Return Strike, Knight's Return Strike) stack; when false, only the first pair is used (classic single-skill behavior).")
...
RULE_CATEGORY(Ascendant)
// --- Pet survivability ---
RULE_REAL(Ascendant, PetHPScalar, 1.5,
...
RULE_INT(Ascendant, PureCasterDmgMinCastTimeMS, 2500,
	"[Ascendant] Minimum spell cast_time (ms) required for PureCasterSpellDmgBonus to apply.")
// --- Raid target bonus ---
RULE_INT(Ascendant, PureCasterRaidTargetDmgBonus, 25,
	"[Ascendant] Additional pct bonus to nuke AND DoT damage for Nec/Wiz/Mag/Enc vs IsRaidTarget-flagged mobs. Stacks with PureCasterSpellDmgBonus. 0 = disabled.")
// --- Caster AC softcap ---
RULE_INT(Ascendant, CasterACSoftcapOffset, 40,
	"[Ascendant] Flat value added to the Nec/Wiz/Mag/Enc AC softcap array in GetACSoftcap(). 0 = no change.")
// --- Sympathetic proc gates ---
RULE_INT(Ascendant, SympatheticProcMinMana, 10,
	"[Ascendant] Minimum mana cost of the triggering spell for sympathetic procs to fire. Blocks item clicks (mana=0). 0 = disabled.")
RULE_INT(Ascendant, SympatheticProcMinCastTimeMS, 1000,
	"[Ascendant] Minimum cast_time (ms) of the triggering spell for sympathetic procs to fire. Blocks instant-cast clickies. 0 = disabled.")
// --- DoT caster damage bonuses ---
RULE_INT(Ascendant, DoTCasterSpellDmgBonus, 35,
	"[Ascendant] Pct bonus to DoT damage for Nec/Dru/Sha. Applied to both crit and non-crit paths. Excludes lifetaps. cast_time >= 2000ms gate. 0 = disabled.")
RULE_INT(Ascendant, DoTCasterRaidTargetDmgBonus, 20,
	"[Ascendant] Additional pct bonus to DoT damage vs IsRaidTarget-flagged mobs for Nec/Dru/Sha. Stacks with DoTCasterSpellDmgBonus. 0 = disabled.")
RULE_INT(Ascendant, BardSongDmgBonus, 20,
	"[Ascendant] Pct bonus to bard damage song tic damage in GetActDoTDamage. Separate from DoTCasterSpellDmgBonus. 0 = disabled.")
// --- Hybrid caster nuke bonus ---
RULE_INT(Ascendant, HybridCasterSpellDmgBonus, 25,
	"[Ascendant] Pct bonus to nuke damage for Dru/Sha on spells with cast_time >= PureCasterDmgMinCastTimeMS. Excludes lifetaps. 0 = disabled.")
// --- DoT cast time cap ---
