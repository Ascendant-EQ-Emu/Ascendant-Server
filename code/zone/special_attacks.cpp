...
void Client::OPCombatAbility(const CombatAbility_Struct *ca_atk)
{
	...
	auto double_special_chance = [&]() -> int {
		return itembonuses.DoubleSpecialAttack +
		       spellbonuses.DoubleSpecialAttack +
		       aabonuses.DoubleSpecialAttack;
	};

	auto roll_extra_special_attacks = [&](int chance) -> int {
		int extra = 0;
		while (chance > 0) {
	...
	if (
		...
		if (GetTarget() != this) {
			...
			// SPA 283: allow DoubleSpecialAttack to grant extra Bash/Slam attempts (non-monk)
			if (class_id != Class::Monk) {
				int chance = double_special_chance();
				if (chance > 0) {
					int extra = roll_extra_special_attacks(chance);
					extra = EQ::Clamp(extra, 0, 1);

					while (extra-- > 0) {
						if (!GetTarget() || !IsAttackAllowed(GetTarget()) || !CombatRange(GetTarget())) {
							break;
						}

						int64 extra_damage = 0;
						int extra_hate_override = 0;
						...
						if (
							...
							extra_hate_override = extra_damage = GetBaseSkillDamage(EQ::skills::SkillBash, GetTarget());
						}

						DoSpecialAttackDamage(GetTarget(), EQ::skills::SkillBash, extra_damage, 0, extra_hate_override, 0);
					}
				}
			}
	...
	if (ca_atk->m_atk == 100 && ca_atk->m_skill == EQ::skills::SkillFrenzy) {
		...
		// SPA 283: allow DoubleSpecialAttack to grant extra Frenzy attempts (non-monk)
		if (class_id != Class::Monk) {
			int chance = double_special_chance();
			if (chance > 0) {
				int extra = roll_extra_special_attacks(chance);
				...
				while (extra-- > 0) {
					...
					int extra_max_dmg = GetBaseSkillDamage(EQ::skills::SkillFrenzy, GetTarget());

					const EQ::ItemInstance* primary_in_use = GetInv().GetItem(EQ::invslot::slotPrimary);
					if (primary_in_use && GetWeaponDamage(GetTarget(), primary_in_use) <= 0) {
						extra_max_dmg = DMG_INVULNERABLE;
					}

					DoSpecialAttackDamage(GetTarget(), EQ::skills::SkillFrenzy, extra_max_dmg, 0, extra_max_dmg, 0);
				}
			}
		}
	...
	if (
		...
		if (GetTarget() != this) {
			...
			// SPA 283: allow DoubleSpecialAttack to grant extra Kick attempts (non-monk)
			if (class_id != Class::Monk) {
				int chance = double_special_chance();
				if (chance > 0) {
					int extra = roll_extra_special_attacks(chance);
					extra = EQ::Clamp(extra, 0, 2);
					...
					while (extra-- > 0) {
						...
						if (GetWeaponDamage(GetTarget(), GetInv().GetItem(EQ::invslot::slotFeet)) <= 0) {
							...
							extra_hate_override = extra_damage = GetBaseSkillDamage(EQ::skills::SkillKick, GetTarget());
						}

						DoSpecialAttackDamage(GetTarget(), EQ::skills::SkillKick, extra_damage, 0, extra_hate_override, 0);
					}
				}
			}
	...
	if (
		...
		// SPA 283: allow DoubleSpecialAttack to grant extra Backstab attempts
		int chance = double_special_chance();
		if (chance > 0) {
			int extra = roll_extra_special_attacks(chance);
			extra = EQ::Clamp(extra, 0, 2);
