...
int64 Mob::GetActSpellDamage(uint16 spell_id, int64 value, Mob* target) {
	...
	if (chance > 0 || (IsOfClientBot() && GetClass() == Class::Wizard && GetLevel() >= RuleI(Spells, WizCritLevel))) {
		...
		if (Critical) {
			...
			if (RuleI(Ascendant, PureCasterSpellDmgBonus) > 0
				...
				value = value * (100 + RuleI(Ascendant, PureCasterSpellDmgBonus)) / 100;
			}
			// [Ascendant] Pure caster raid target damage bonus (crit path)
			if (RuleI(Ascendant, PureCasterRaidTargetDmgBonus) > 0
				&& target && target->IsRaidTarget()
				&& IsOfClientBot()
				&& EQ::ValueWithin(GetClass(), Class::Necromancer, Class::Enchanter)) {
				value = value * (100 + RuleI(Ascendant, PureCasterRaidTargetDmgBonus)) / 100;
			}
			// [Ascendant] Hybrid caster nuke bonus for Dru/Sha/Clr (crit path)
			if (RuleI(Ascendant, HybridCasterSpellDmgBonus) > 0
				&& IsOfClientBot()
				&& (GetClass() == Class::Druid || GetClass() == Class::Shaman || GetClass() == Class::Cleric)
				&& !spells[spell_id].no_heal_damage_item_mod
				&& spells[spell_id].cast_time >= RuleI(Ascendant, PureCasterDmgMinCastTimeMS)) {
	...
	// [Ascendant] Pure caster raid target damage bonus
	if (RuleI(Ascendant, PureCasterRaidTargetDmgBonus) > 0
		&& target && target->IsRaidTarget()
		&& IsOfClientBot()
		&& EQ::ValueWithin(GetClass(), Class::Necromancer, Class::Enchanter)) {
	...
	// [Ascendant] Hybrid caster nuke bonus for Dru/Sha/Clr (non-crit path)
	if (RuleI(Ascendant, HybridCasterSpellDmgBonus) > 0
		&& IsOfClientBot()
		&& (GetClass() == Class::Druid || GetClass() == Class::Shaman || GetClass() == Class::Cleric)
		&& !spells[spell_id].no_heal_damage_item_mod
...
int64 Mob::GetActDoTDamage(uint16 spell_id, int64 value, Mob* target, bool from_buff_tic) {
	...
	if (!spells[spell_id].good_effect && chance > 0 && (zone->random.Roll(chance))) {
		...
		// [Ascendant] DoT caster damage bonus (crit path)
		if (RuleI(Ascendant, DoTCasterSpellDmgBonus) > 0
			&& IsOfClientBot()
			&& !spells[spell_id].no_heal_damage_item_mod
			&& spells[spell_id].cast_time >= 2000
			&& (GetClass() == Class::Necromancer || GetClass() == Class::Druid || GetClass() == Class::Shaman)) {
			value = value * (100 + RuleI(Ascendant, DoTCasterSpellDmgBonus)) / 100;
		}
		// [Ascendant] DoT caster raid target bonus (crit path)
		if (RuleI(Ascendant, DoTCasterRaidTargetDmgBonus) > 0
			&& target && target->IsRaidTarget()
			&& IsOfClientBot()
			&& (GetClass() == Class::Necromancer || GetClass() == Class::Druid || GetClass() == Class::Shaman)) {
			value = value * (100 + RuleI(Ascendant, DoTCasterRaidTargetDmgBonus)) / 100;
		}
		// [Ascendant] Pure caster raid target DoT bonus (crit path)
		if (RuleI(Ascendant, PureCasterRaidTargetDmgBonus) > 0
			&& target && target->IsRaidTarget()
			&& IsOfClientBot()
			&& EQ::ValueWithin(GetClass(), Class::Necromancer, Class::Enchanter)) {
		...
		// [Ascendant] Bard song damage bonus (crit path)
		if (RuleI(Ascendant, BardSongDmgBonus) > 0
			&& IsBardSong(spell_id)
			&& IsOfClientBot()) {
		...
		// [Ascendant] DoT caster damage bonus (non-crit path)
		if (RuleI(Ascendant, DoTCasterSpellDmgBonus) > 0
			&& IsOfClientBot()
			&& !spells[spell_id].no_heal_damage_item_mod
			&& spells[spell_id].cast_time >= 2000
		...
		// [Ascendant] DoT caster raid target bonus (non-crit path)
		if (RuleI(Ascendant, DoTCasterRaidTargetDmgBonus) > 0
			&& target && target->IsRaidTarget()
			&& IsOfClientBot()
			&& (GetClass() == Class::Necromancer || GetClass() == Class::Druid || GetClass() == Class::Shaman)) {
		...
		// [Ascendant] Pure caster raid target DoT bonus (non-crit path)
		if (RuleI(Ascendant, PureCasterRaidTargetDmgBonus) > 0
			&& target && target->IsRaidTarget()
			&& IsOfClientBot()
			&& EQ::ValueWithin(GetClass(), Class::Necromancer, Class::Enchanter)) {
		...
		// [Ascendant] Bard song damage bonus (non-crit path)
		if (RuleI(Ascendant, BardSongDmgBonus) > 0
			&& IsBardSong(spell_id)
			&& IsOfClientBot()) {
			value = value * (100 + RuleI(Ascendant, BardSongDmgBonus)) / 100;
