...
void Mob::MakePoweredPet(uint16 spell_id, const char* pettype, int16 petpower,
	...
	// [Ascendant] Scale base pet HP and AC via rules before any other modifications
	if (RuleR(Ascendant, PetHPScalar) != 1.0f) {
		npc_type->max_hp     = static_cast<int64>(npc_type->max_hp * RuleR(Ascendant, PetHPScalar));
		npc_type->current_hp = npc_type->max_hp;
	}
	if (RuleR(Ascendant, PetACScalar) != 1.0f) {
		npc_type->AC = static_cast<int32>(npc_type->AC * RuleR(Ascendant, PetACScalar));
	}
