	...
	if (cast_time <= -1) {
		...
		if (cast_time) {
			...
			if (RuleI(Ascendant, DoTCastTimeCapMS) > 0
				&& cast_time > RuleI(Ascendant, DoTCastTimeCapMS)
				&& IsOfClientBot()
				&& IsDamageOverTimeSpell(spell_id)
				&& (GetClass() == Class::Necromancer || GetClass() == Class::Druid
					|| GetClass() == Class::Shaman || GetClass() == Class::Bard))
				cast_time = RuleI(Ascendant, DoTCastTimeCapMS);
		}
	}
	...
	else // not bard, check movement
	{
		...
		)
		{
			...
			if (IsOfClientBot()) {
				...
				// [Ascendant] Pure caster channeling bonus
				if (RuleI(Ascendant, PureCasterChannelingBonus) > 0
					&& EQ::ValueWithin(GetClass(), Class::Necromancer, Class::Enchanter))
					channelchance += RuleI(Ascendant, PureCasterChannelingBonus);
			} else {
