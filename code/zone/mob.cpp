...
void Mob::TrySympatheticProc(Mob* target, uint32 spell_id)
{
	...
	// [Ascendant] Block clickies and instant-cast spells from triggering sympathetic procs
	if (RuleI(Ascendant, SympatheticProcMinMana) > 0 && spells[spell_id].mana < RuleI(Ascendant, SympatheticProcMinMana))
		return;
	if (RuleI(Ascendant, SympatheticProcMinCastTimeMS) > 0 && spells[spell_id].cast_time < RuleI(Ascendant, SympatheticProcMinCastTimeMS))
		return;
