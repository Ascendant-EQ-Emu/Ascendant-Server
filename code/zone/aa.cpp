...
void Client::ResetAA()
{
	...
	for (auto& rank_value: aa_ranks) {
		...
		if (slot_id >= MAX_PP_AA_ARRAY) {
			LogError("Client [{}] exceeded MAX_PP_AA_ARRAY [{}] during ResetAA, skipping remaining", GetCleanName(), MAX_PP_AA_ARRAY);
			break;
		}
		...
		if(ranks) {
			...
			if(rank) {
				if (i >= MAX_PP_AA_ARRAY) {
					LogError("Client [{}] exceeded MAX_PP_AA_ARRAY [{}] in SendAlternateAdvancementPoints, skipping remaining", GetCleanName(), MAX_PP_AA_ARRAY);
					break;
				}
...
bool ZoneDatabase::LoadAlternateAdvancement(Client *c) {
	...
	for (const auto& e : l) {
		...
		if (c->CanUseAlternateAdvancementRank(rank)) {
			if (slot_id >= MAX_PP_AA_ARRAY) {
				LogError("Character [{}] exceeded MAX_PP_AA_ARRAY [{}], skipping remaining AAs", c->CharacterID(), MAX_PP_AA_ARRAY);
				break;
			}
